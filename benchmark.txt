...
if (php_sapi_name() == "cli") {
    ...
    display($benchmarkResult, $showServerName);
}
...
function test_benchmark($settings)
{
    ...
    $count = $settings['test_count'];

    test_math($result, $count);
    test_string($result, $count);
    test_loops($result, $count*100);
    test_ifelse($result, $count*100);

    $result['benchmark']['calculation_total'] = timer_diff($timeStart) . ' sec.';
    if (!empty($settings['test.db'])) {
...
function test_loops(&$result, $count = 99999)
{
    $timeStart = microtime(true);
    for ($i = 0; $i < $count; ++$i) {
    }
...
function test_ifelse(&$result, $count = 99999)
{
    $timeStart = microtime(true);
    for ($i = 0; $i < $count; $i++) {
        if ($i == -1) {
 ...
 * Display Methods
 */
function display($benchmarkResult, $showServerName)
{
    // html output
    echo "<!DOCTYPE html>\n<html><head>\n";
    echo "<style>
